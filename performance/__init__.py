...
class PerformanceService:
    ...
    @staticmethod
    def calculate_completion_speed(
        deadline: datetime, completion_time: datetime
    ) -> float:
        """
    ...
    def calculate_on_time_ratio(user_id: int) -> float:
        ...
        try:
            ...
            logger.error(
                "Error calculating on-time ratio for user %s: %s", user_id, exc
            )
            return 0.5
    ...
    def calculate_skill_accuracy(user_id: int) -> float:
        ...
        try:
            ...
            logger.error(
                "Error calculating skill accuracy for user %s: %s", user_id, exc
            )
            return 0.5
    ...
    def calculate_difficulty_factor(user_id: int) -> float:
        ...
        try:
            ...
            logger.error(
                "Error calculating difficulty factor for user %s: %s", user_id, exc
            )
            return 0.5

    @staticmethod
    def calculate_performance_score(
        on_time_ratio: float, skill_accuracy: float, difficulty_factor: float
    ) -> float:
        """
        ...
        score = (
            on_time_ratio * 0.5
            + skill_accuracy * 0.3  # 50% weight on-time completion
            + difficulty_factor  # 30% weight skill accuracy
            * 0.2  # 20% weight difficulty
        ) * 100
    ...
    def update_user_performance(
        user_id: int, assignment_id: Optional[int] = None
    ) -> Dict:
        """
        ...
        try:
            ...
            performance_score = PerformanceService.calculate_performance_score(
                on_time_ratio, skill_accuracy, difficulty_factor
            )
            ...
            avg_completion_time = (
                total_hours / tasks_completed if tasks_completed > 0 else 0
            )
            ...
            for assignment in assignments:
                if assignment.completed_at and assignment.task.due_date:
                    speed = PerformanceService.calculate_completion_speed(
                        assignment.task.due_date, assignment.completed_at
                    )
                    speeds.append(speed)
    ...
    def get_user_performance_history(user_id: int, days: int = 30) -> List[Dict]:
        ...
        try:
            ...
            logger.error(
                "Error getting performance history for user %s: %s", user_id, exc
            )
            return []
    ...
    def get_user_performance_summary(user_id: int) -> Dict:
        ...
        try:
                ...
                "latest_metrics": (
                    {
                        ...
                        "skill_accuracy": (
                            latest_log.skill_accuracy if latest_log else 0
                        ),
                        "difficulty_factor": (
                            latest_log.difficulty_factor if latest_log else 0
                        ),
                        "avg_completion_speed": (
                            latest_log.avg_completion_speed if latest_log else 0
                        ),
                    }
                    if latest_log
                    else None
            ...
            logger.error(
                "Error getting performance summary for user %s: %s", user_id, exc
            )
    ...
    def get_performance_trends(user_id: int, days: int = 90) -> Dict:
        ...
        try:
            ...
            logger.error(
                "Error getting performance trends for user %s: %s", user_id, exc
            )
            return {"error": str(exc)}
