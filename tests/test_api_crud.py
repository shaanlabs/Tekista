...
def get_token(client, app):
    ...
    resp = client.post('/api/token', json={'username': 'apiuser', 'password': 'pw123456'})
...
def test_api_projects_crud(client, app):
    ...
    # Create
    resp = client.post('/api/projects', headers=auth_headers(token), json={'title': 'API Project', 'description': 'd'})
    ...
    # Update
    resp = client.patch(f'/api/projects/{pid}', headers=auth_headers(token), json={'description': 'updated'})
...
def test_api_tasks_crud(client, app):
    ...
    # Create
    resp = client.post('/api/tasks', headers=auth_headers(token), json={'title': 'T1', 'project_id': pid})
    assert resp.status_code == 201
    ...
    # Update
    resp = client.patch(f'/api/tasks/{tid}', headers=auth_headers(token), json={'status': 'Done'})
