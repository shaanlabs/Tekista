...
def test_csv_export_rbac_and_audit(client, app):
    ...
    with app.app_context():
        # seed users
        _admin = make_user(
            "admin_test", "admin_test@example.com", "pw123456", admin_role
        )
        _member = make_user(
            "member_test", "member_test@example.com", "pw123456", member_role
        )
    ...
    with app.app_context():
        entry = (
            AuditLog.query.filter_by(action="export_csv", target_id=p.id)
...
def test_files_upload_delete_audit(client, app, tmp_path):
    ...
    with app.app_context():
        _admin = make_user(
            "file_admin", "file_admin@example.com", "pw123456", admin_role
        )
    ...
    # upload
    data = {"project_id": str(pid), "file": (io.BytesIO(b"hello"), "hello.txt")}
    resp = client.post("/files/upload", data=data, content_type="multipart/form-data")
    assert resp.status_code in (302, 303)

    # delete
    resp = client.post(
        "/files/delete", data={"project_id": str(pid), "filename": "hello.txt"}
    )
    assert resp.status_code in (302, 303)
...
def test_admin_users_filters(client, app):
    ...
    with app.app_context():
        # seed data
        _admin = make_user(
            "admin_filter", "admin_filter@example.com", "pw123456", admin_role
        )
