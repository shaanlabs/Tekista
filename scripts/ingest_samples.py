...
for k in [
    "anomaly_engine_enabled",
    "reliability_score_enabled",
    "feature_rollups_enabled",
    "baseline_refresh_enabled",
]:
...
for k, v in [("time_logged_coding_hours", 7.0), ("time_logged_total_hours", 8.0)]:
    rec = UserDailyFeature.query.filter_by(
        user_id=USER_ID, date=THE_DATE, feature_key=k
    ).first()
    if not rec:
        rec = UserDailyFeature(
            user_id=USER_ID, date=THE_DATE, feature_key=k, value=0.0, source="time"
        )
        db.session.add(rec)
    rec.value = float(v)
...
rec = UserDailyFeature.query.filter_by(
    user_id=USER_ID, date=THE_DATE, feature_key="commits_count"
).first()
if not rec:
    rec = UserDailyFeature(
        user_id=USER_ID,
        date=THE_DATE,
        feature_key="commits_count",
        value=0.0,
        source="vcs",
    )
    db.session.add(rec)
...
rec = UserDailyFeature.query.filter_by(
    user_id=USER_ID, date=THE_DATE, feature_key="meeting_hours"
).first()
if not rec:
    rec = UserDailyFeature(
        user_id=USER_ID,
...
print(
    "Anomaly detectors will evaluate within 10-15 minutes; check /admin/anomalies or GET /api/anomalies?user_id=",
    USER_ID,
)
