...
def upsert_user(session, u):
    ...
    if not user:
        ...
        session.commit()
    # role
    role_name = 'Admin' if u['role'] == 'Admin' else ('Manager' if u['role'] in ('Manager','Project Manager') else 'Member')
...
def main():
    ap = argparse.ArgumentParser(description='Load seed E2E JSON into app (API or DB modes)')
    ...
    ap.add_argument('--skip-users', action='store_true', help='[Deprecated] Same as --no-upsert-users')
    ap.add_argument('--no-upsert-users', action='store_true', help='API mode: do not upsert users via DB (requires users to already exist)')
    args = ap.parse_args()
    ...
    if args.mode == 'db':
        ...
        for p in seed.get('projects', []):
            existed = _Project.query.filter_by(title=p['projectName']).first() is not None
        ...
        for t in seed.get('tasks', []):
            proj = _Project.query.filter_by(id=t['projectId']).first() or _Project.query.filter_by(title=f"Project {t['projectId']}").first()
            if not proj:
                continue
            # avoid duplicate by title+project
            existing = _Task.query.filter_by(title=t['taskTitle'], project_id=proj.id).first()
            if existing:
                continue
            new_t = _Task(title=t['taskTitle'], description='', priority=t.get('priority','Medium'), project_id=proj.id, status=t.get('status','To-Do'))
            _db.session.add(new_t)
            _db.session.commit()
            created_t += 1
        print(f"DB mode: users created={created_u}, updated={updated_u}; projects created={created_p}, updated={updated_p}; tasks created={created_t}")
        return
    ...
    if not args.admin_username or not args.admin_password:
        print('Provide --admin-username and --admin-password for API mode', file=sys.stderr)
        sys.exit(2)
    ...
    for t in seed.get('tasks', []):
        ...
        created_t += 1
    print(f"API mode: users upserted via DB={len(seed.get('users', [])) if not args.skip_users else 0}; projects upserted={len(project_id_map)}; tasks created={created_t}")
