...
with app.app_context():
    ...
    try:
        conn.execute(text('UPDATE user SET current_workload=0 WHERE current_workload IS NULL'))
    except SQLAlchemyError as err:
        print('seed workload skip', err)
    try:
        conn.execute(text('UPDATE user SET availability="Available" WHERE availability IS NULL OR availability=""'))
    except SQLAlchemyError as err:
        print('seed availability skip', err)
    try:
        conn.execute(text('UPDATE task SET estimated_hours=4.0 WHERE estimated_hours IS NULL'))
    except SQLAlchemyError as err:
