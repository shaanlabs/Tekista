...
def get_unread_count():
    ...
    return jsonify({"user_id": current_user.id, "unread_count": count}), 200
...
def mark_as_read(notification_id):
    ...
    return (
        jsonify({"success": True, "notification_id": notification_id, "is_read": True}),
        200,
    )
...
def get_notification_statistics():
    ...
    # Count by type
    types = (
        db.session.query(Notification.notification_type, db.func.count(Notification.id))
