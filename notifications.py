...
def send_email(subject, recipients, body):
	...
	if not recipients:
		return
	try:
		msg = Message(subject=subject, recipients=recipients, body=body, sender=current_app.config.get('MAIL_DEFAULT_SENDER'))
		mail.send(msg)
	except Exception:
		# fail silently (or log) if mail not configured or send fails
		current_app.logger.debug("Email send failed or mail not configured.", exc_info=True)
