...
class UserSkillProfile(db.Model):
    ...
    __table_args__ = (
        Index('idx_user_skill_profile_user_id', 'user_id'),
    )
...
class TaskAssignment(db.Model):
    ...
    assigned_by_id = db.Column(db.Integer, db.ForeignKey('user.id'))  # NULL for auto-assignment
    
    # Assignment strategy and scores
    assignment_strategy = db.Column(db.String(50))  # skill_match, workload_balance, performance, hybrid
    ...
    # Status
    assignment_status = db.Column(db.String(50), default='active')  # active, completed, cancelled
...
class SkillEndorsement(db.Model):
    ...
    # Relationships
    user = db.relationship('User', foreign_keys=[user_id], backref='skill_endorsements_received')
    ...
    __table_args__ = (
        ...
        db.UniqueConstraint('user_id', 'endorsed_by_id', 'skill', name='unique_endorsement'),
    )
...
class AssignmentFeedback(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    assignment_id = db.Column(db.Integer, db.ForeignKey('task_assignment.id'), nullable=False)
    ...
    __table_args__ = (
        Index('idx_assignment_feedback_assignment_id', 'assignment_id'),
    )
...
class AssignmentStatistics(db.Model):
    ...
    __table_args__ = (
        Index('idx_assignment_statistics_user_id', 'user_id'),
    )
    ...
    def update_metrics(self):
        ...
        assignments = TaskAssignment.query.filter_by(assigned_user_id=self.user_id).all()
        
        if not assignments:
            return
        
        self.total_assignments = len(assignments)
        self.completed_assignments = sum(1 for a in assignments if a.assignment_status == 'completed')
        self.cancelled_assignments = sum(1 for a in assignments if a.assignment_status == 'cancelled')
        
        # Calculate average metrics
        completed = [a for a in assignments if a.assignment_status == 'completed']
        
        if completed:
            self.avg_skill_match_score = sum(a.skill_match_score for a in completed) / len(completed)
            self.avg_completion_time = sum(a.actual_completion_hours or 0 for a in completed) / len(completed)
            
            # Accuracy
            accurate = [a for a in completed if a.actual_completion_hours and a.estimated_completion_hours]
            if accurate:
                self.avg_estimation_accuracy = sum(a.get_accuracy_ratio() for a in accurate) / len(accurate)
