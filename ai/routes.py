...
from models import Task, User, task_assignees
...
def create_task_from_nl():
    ...
    result = ai_assistant.process_natural_language_task(
        text, project_id=project_id, user_id=current_user.id
    )
...
def chat():
    ...
    try:
        ...
        tasks = (
            ...
            .filter(task_assignees.c.user_id == user.id, Task.status != "Completed")
        ...
        if provider == "ollama":
            ...
            result = client.chat(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a helpful task management assistant. Use the provided context. Be concise and helpful.\n\n"
                        + context,
                    },
        ...
        if not openai:
            return (
                jsonify(
                    {"error": "OpenAI library not installed and Ollama not selected"}
                ),
                500,
            )
        ...
        if not api_key:
            return (
                jsonify(
                    {"error": "OpenAI API key not configured and Ollama not selected"}
                ),
                500,
            )
        ...
        response = openai.ChatCompletion.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a helpful task management assistant. Use the following context to provide relevant responses. Be concise and helpful.\n\n"
                    + context,
                },
                {"role": "user", "content": message},
            ],
