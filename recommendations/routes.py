...
def get_recommendation_analysis():
    ...
    recommendations = RecommendationEngine.recommend_tasks_for_user(
        current_user.id, top_n, current_user.organization_id
    )
    ...
    for rec in recommendations:
        ...
        if components["skill_overlap"] > 0.7:
            explanation.append(
                f"Strong skill match ({components['skill_overlap']*100:.0f}%)"
            )
        elif components["skill_overlap"] > 0.4:
            explanation.append(
                f"Moderate skill match ({components['skill_overlap']*100:.0f}%)"
            )

        if components["success_rate"] > 0.8:
            explanation.append(
                f"High success rate on similar tasks ({components['success_rate']*100:.0f}%)"
            )
    ...
    return jsonify({"user_id": current_user.id, "analysis": analysis}), 200
