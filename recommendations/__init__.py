...
class RecommendationEngine:
    ...
    def calculate_skill_overlap(user_id: int, task_id: int) -> float:
        ...
        try:
            ...
            required_skills = set(skill.lower() for skill in (task.required_skills or []))
    ...
    def calculate_completion_time_similarity(user_id: int, task_id: int) -> float:
        ...
        try:
            ...
            # Calculate ratio
            time_ratio = estimated_task_time / user_avg_time if user_avg_time > 0 else 1.0
        ...
        try:
            ...
            if not similar_tasks:
                ...
                on_time_count = sum(
                    ...
                    if a.completed_at and a.task.due_date and a.completed_at <= a.task.due_date
                )
    ...
    def calculate_priority_boost(task_id: int) -> float:
        ...
        try:
            ...
            boost = priority_boost.get(task.priority.lower(), 1.0) if task.priority else 1.0
    ...
    def calculate_recommendation_score(
        ...
        try:
            ...
            skill_overlap = RecommendationEngine.calculate_skill_overlap(user_id, task_id)
            completion_time = RecommendationEngine.calculate_completion_time_similarity(user_id, task_id)
            success_rate = RecommendationEngine.calculate_success_rate_on_similar_tasks(user_id, task_id)
            workload_fit = RecommendationEngine.calculate_workload_fit(user_id, task_id)
            experience_match = RecommendationEngine.calculate_experience_match(user_id, task_id)
    ...
    def recommend_tasks_for_user(
        ...
        try:
            ...
            for task in available_tasks:
                score = RecommendationEngine.calculate_recommendation_score(user_id, task.id)
                ...
                if score > 0:  # Only include tasks with positive score
                    task_scores.append({
                        ...
                        'components': {
                            'skill_overlap': RecommendationEngine.calculate_skill_overlap(user_id, task.id),
                            'completion_time_fit': RecommendationEngine.calculate_completion_time_similarity(user_id, task.id),
                            'success_rate': RecommendationEngine.calculate_success_rate_on_similar_tasks(user_id, task.id),
                            'workload_fit': RecommendationEngine.calculate_workload_fit(user_id, task.id),
                            'experience_match': RecommendationEngine.calculate_experience_match(user_id, task.id)
                        }
                    })
    ...
    def get_personalized_recommendations(
        ...
        try:
            ...
            if user_profile:
                if user_profile.is_overloaded():
                    insights.append("You're currently overloaded. Consider completing some tasks first.")
                
                if user_profile.performance_score < 60:
                    insights.append("Your performance score is below average. Try tasks with lower difficulty.")
                elif user_profile.performance_score > 85:
                    insights.append("Great performance! You can handle more challenging tasks.")
                
                available_capacity = user_profile.available_capacity()
                if available_capacity < 5:
                    insights.append("You have limited capacity. Focus on high-priority tasks.")
