...
class AnalyticsEngine:
    ...
    def get_team_performance_summary(organization_id: int, days: int = 30) -> Dict:
        ...
        try:
            ...
            completed_tasks = db.session.query(TaskAssignment).join(Task).join(Project).filter(
            ...
            for assignment in completed_tasks:
                ...
                if assignment.created_at and assignment.completed_at:
                    time_diff = (assignment.completed_at - assignment.created_at).total_seconds() / 3600
                    total_time_hours += time_diff
            
            total_tasks = len(completed_tasks)
            on_time_ratio = on_time_count / total_tasks if total_tasks > 0 else 0
            avg_completion_time = total_time_hours / total_tasks if total_tasks > 0 else 0
            
            # Calculate team performance score
            team_score = (on_time_ratio * 100) * 0.7 + (100 - min(late_count / total_tasks * 100, 100)) * 0.3
    ...
    def get_task_distribution_by_skill(organization_id: int) -> Dict[str, int]:
        ...
        """
        try:
            tasks = db.session.query(Task).join(Project).filter(
                Project.organization_id == organization_id
            ).all()
    ...
    def get_top_performers(organization_id: int, limit: int = 10) -> List[Dict]:
        ...
        try:
            ...
            users = db.session.query(User).filter_by(organization_id=organization_id).all()
            ...
            for user in users:
                ...
                completed_count = db.session.query(func.count(TaskAssignment.id)).filter_by(
    ...
    def get_task_completion_ratio(organization_id: int, days: int = 30) -> Dict:
        ...
        try:
            ...
            all_tasks = db.session.query(Task).join(Project).filter(
                Project.organization_id == organization_id,
    ...
    @staticmethod
    def get_team_performance_trend(organization_id: int, days: int = 30, interval: str = 'daily') -> List[Dict]:
        """
    ...
    def get_project_statistics(organization_id: int) -> List[Dict]:
        ...
        try:
            ...
            for project in projects:
                ...
                overdue = sum(1 for t in tasks if t.due_date and t.due_date < datetime.utcnow().date() and t.status != 'Completed')
    ...
    def get_productivity_metrics(organization_id: int, days: int = 30) -> Dict:
        ...
        try:
            ...
            # Get users
            users = db.session.query(User).filter_by(organization_id=organization_id).count()
            ...
            perf_logs = db.session.query(PerformanceLog).filter(
                PerformanceLog.created_at >= cutoff_date
            ).all()
            
            avg_performance = sum(log.performance_score for log in perf_logs) / len(perf_logs) if perf_logs else 0
            ...
            return {
                ...
                'productivity_index': (completion_rate / 100) * (avg_performance / 100) * 100
            }
                ...
                if profile and profile.skills:
                    for skill, proficiency in profile.skills.items():
                        ...
                        skill_distribution[skill]['max_proficiency'] = max(
                            skill_distribution[skill]['max_proficiency'],
                            proficiency
                        )
                        skill_distribution[skill]['min_proficiency'] = min(
                            skill_distribution[skill]['min_proficiency'],
                            proficiency
                        )
    ...
    def get_comprehensive_analytics(organization_id: int, days: int = 30) -> Dict:
        ...
        try:
            return {
                ...
                'team_performance': AnalyticsEngine.get_team_performance_summary(organization_id, days),
                'task_completion': AnalyticsEngine.get_task_completion_ratio(organization_id, days),
                'productivity': AnalyticsEngine.get_productivity_metrics(organization_id, days),
                'top_performers': AnalyticsEngine.get_top_performers(organization_id, limit=5),
                'task_distribution': AnalyticsEngine.get_task_distribution_by_skill(organization_id),
                'performance_trend': AnalyticsEngine.get_team_performance_trend(organization_id, days),
                'projects': AnalyticsEngine.get_project_statistics(organization_id),
                'skill_distribution': AnalyticsEngine.get_skill_proficiency_distribution(organization_id)
            }
