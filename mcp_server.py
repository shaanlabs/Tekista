...
class TaskManagerMCPServer:
            ...
            return [
                {
                    ...
                    "assignees": [u.username for u in t.assignees],
                    "created_at": (
                        t.created_at.isoformat() if hasattr(t, "created_at") else None
                    ),
                }
                for t in tasks
            ]
    ...
    def get_task_details(self, task_id: int) -> dict:
        ...
        try:
            ...
            return {
                ...
                "predecessors": [
                    {"id": p.id, "title": p.title} for p in task.predecessors
                ],
            ...
            return [
                {
                    ...
                    "project_title": t.project.title if t.project else None,
                    "days_until_due": (
                        (t.due_date - datetime.utcnow().date()).days
                        if t.due_date
                        else None
                    ),
                }
    ...
    def get_overdue_tasks(self, project_id: Optional[int] = None) -> list:
        ...
        try:
            query = Task.query.filter(
                Task.status != "Done", Task.due_date < datetime.utcnow().date()
            )
    ...
    def get_team_workload(self, project_id: Optional[int] = None) -> list:
        ...
        try:
            ...
            return sorted(
                workload_data, key=lambda x: x["workload_score"], reverse=True
            )
        except SQLAlchemyError as e:
        ...
        try:
            ...
            return {
                ...
                "high_priority_pending": sum(
                    1 for t in pending_tasks if t.priority == "High"
                ),
                "upcoming_tasks_7_days": len(
                    [
                        t
                        for t in pending_tasks
                        if t.due_date
                        and datetime.utcnow().date()
                        <= t.due_date
    ...
    def handle_request(self, method: str, params: dict) -> Any:
        ...
        handlers = {
            ...
            "get_project_details": lambda: self.get_project_details(
                params.get("project_id")
            ),
            "get_tasks": lambda: self.get_tasks(
                params.get("project_id"), params.get("status"), params.get("user_id")
            ),
            "get_task_details": lambda: self.get_task_details(params.get("task_id")),
            "get_user_tasks": lambda: self.get_user_tasks(
                params.get("user_id"), params.get("status")
            ),
            "get_overdue_tasks": lambda: self.get_overdue_tasks(
                params.get("project_id")
            ),
            "get_upcoming_tasks": lambda: self.get_upcoming_tasks(
                params.get("days", 7), params.get("project_id")
            ),
            "get_project_statistics": lambda: self.get_project_statistics(
                params.get("project_id")
            ),
            "get_team_workload": lambda: self.get_team_workload(
                params.get("project_id")
            ),
            "search_tasks": lambda: self.search_tasks(
                params.get("query"), params.get("project_id")
            ),
            "get_dashboard_summary": lambda: self.get_dashboard_summary(
                params.get("user_id")
            ),
        }
