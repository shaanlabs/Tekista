    ...
    try:
        ...
        try:
            ...
            return {
                "success": False,
                "skipped": True,
                "reason": "assignment_module_unavailable",
            }
        ...
        recommendations = service.get_assignment_recommendations(
            task_id=None, organization_id=user.organization_id, top_n=1
        )
        ...
        result = service.auto_assign_task(
            task_id=best_task_data["task_id"], organization_id=user.organization_id
        )
...
def find_and_assign_tasks_for_available_users(organization_id):
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "Enterprise/assignment modules not available; skipping team auto-assignment"
            )
            return {
                "success": False,
                "skipped": True,
                "reason": "enterprise_modules_unavailable",
            }
        ...
        logger.info(
            f"Assigned tasks to {assigned_count} users in organization {organization_id}"
        )
...
def update_user_performance_metrics(self, user_id, assignment_id):
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "Assignment models not available; skipping performance update"
            )
            return {
                "success": False,
                "skipped": True,
                "reason": "assignment_models_unavailable",
            }
        ...
        estimation_accuracy = (
            assignment.get_accuracy_ratio()
            if assignment.actual_completion_hours
            else 0.5
        )
        ...
        new_score = (
            old_score * 0.6
            + estimation_accuracy * 20  # 60% weight to previous score
            + on_time * 10  # 20% weight to estimation accuracy
            + skill_match  # 10% weight to on-time completion
            * 10  # 10% weight to skill match
        )
        ...
        logger.info(
            f"Updated performance metrics for user {user_id}: {old_score:.1f} → {new_score:.1f}"
        )
        ...
        logger.error(
            f"Error updating performance metrics for user {user_id}: {str(exc)}"
        )
...
def recalculate_team_performance(organization_id):
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "Enterprise/assignment models not available; skipping team performance calc"
            )
            return {
                "success": False,
                "skipped": True,
                "reason": "enterprise_models_unavailable",
            }
        ...
        logger.info(
            f"Recalculated performance for {updated_count} users in organization {organization_id}"
        )
...
def send_task_assignment_notification(user_id, task_id, task_title, skill_match):
    ...
    try:
        ...
        send_notification(
            ...
            data={"task_id": task_id, "skill_match": skill_match},
        )
...
def send_performance_update_notification(user_id, old_score, new_score):
    ...
    try:
        ...
        message = (
            f"{emoji} Performance score updated: {old_score:.1f} → {new_score:.1f}"
        )
...
def generate_daily_performance_reports():
    ...
    try:
        ...
        logger.info(
            f"Generated performance reports for {len(organizations)} organizations"
        )
