        ...
        jsonify(
            {
                ...
                "skills": sorted(
                    skills_with_levels, key=lambda x: x["proficiency"], reverse=True
                ),
            }
        ),
        200,
...
def get_skills_by_category():
    ...
    for category, skills in categorized.items():
        result[category] = [
            ...
            for skill_name, proficiency in sorted(
                skills.items(), key=lambda x: x[1], reverse=True
            )
        ]

    return jsonify({"user_id": current_user.id, "categories": result}), 200
...
def add_skill():
    ...
    if success:
        return (
            jsonify({"success": True, "skill": skill_name, "proficiency": proficiency}),
            201,
        )
    else:
...
def increment_skill(skill_name):
    ...
    new_proficiency = SkillManager.increment_skill(
        current_user.id, skill_name, increment
    )
...
def get_skill_gaps():
    ...
    return jsonify({"user_id": current_user.id, "gaps": gaps, "total": len(gaps)}), 200
...
def get_skill_categories():
    ...
    return jsonify({"categories": SkillManager.SKILL_CATEGORIES}), 200
        ...
        jsonify(
            {
                ...
                "user_skills_in_category": sum(
                    1 for s in category_skills if s["has_skill"]
                ),
            }
        ),
        200,
