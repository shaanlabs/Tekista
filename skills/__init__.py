...
class SkillManager:
    ...
    @staticmethod
    def initialize_user_skills(
        user_id: int, initial_skills: Optional[Dict[str, float]] = None
    ) -> Dict[str, float]:
        """
    ...
    def add_skill(user_id: int, skill_name: str, proficiency: float = 1.0) -> bool:
        ...
        try:
            ...
            # Add or update skill
            profile.skills[skill_name] = max(
                proficiency, profile.skills.get(skill_name, 0)
            )
    ...
    def increment_skill(user_id: int, skill_name: str, increment: float = 1.0) -> float:
        ...
        try:
            ...
            logger.info(
                "Incremented '%s' for user %s: %.1f → %.1f",
                skill_name,
                user_id,
                current,
                new_value,
    ...
    @staticmethod
    def update_skills_from_completed_task(
    ...
    def get_skill_gaps(user_id: int) -> List[Dict]:
        ...
        try:
            ...
            for skill, tasks in skill_requirements.items():
                ...
                if proficiency < 30:
                    avg_difficulty = (
                        sum(t["difficulty"] for t in tasks) / len(tasks) if tasks else 0
                    )
    ...
    def get_learning_path(user_id: int) -> Dict:
        ...
        try:
            ...
            return {
                "user_id": user_id,
                "top_skills": [
                    {"skill": s[0], "proficiency": s[1]} for s in top_skills
                ],
                "recommendations": recommendations,
                "skill_gaps": gaps[:3],
                "suggested_focus": (
                    recommendations[0]["skill"] if recommendations else None
                ),
