...
def create_organization():
    ...
    return jsonify({"id": org.id, "name": org.name, "slug": org.slug}), 201
...
def create_role(org_id):
    ...
    return (
        jsonify({"id": role.id, "name": role.name, "permissions": role.permissions}),
        201,
    )
...
@enterprise_bp.route(
    "/organizations/<int:org_id>/members/<int:user_id>", methods=["PUT"]
)
@login_required
@require_role("admin")
...
@enterprise_bp.route(
    "/organizations/<int:org_id>/api-tokens/<int:token_id>", methods=["DELETE"]
)
@login_required
...
def create_oauth_config(org_id):
    ...
    existing = OAuthConfig.query.filter_by(
        organization_id=org_id, provider=data.get("provider")
    ).first()
    ...
    return jsonify({"id": config.id, "provider": config.provider}), 201
...
def oauth_authorize(provider):
    ...
    config = OAuthConfig.query.filter_by(
        organization_id=org_id, provider=provider, is_enabled=True
    ).first_or_404()
    ...
    if provider == "google":
        oauth_provider = GoogleOAuthProvider(
            config.client_id, client_secret, config.redirect_uri
        )
...
def oauth_callback(provider):
    ...
    oauth_account = OAuthAccount.query.filter_by(
        provider=provider, provider_user_id=user_info.get("id") or user_info.get("sub")
    ).first()
    ...
    if oauth_account:
        ...
        user_org_role = UserOrganizationRole(
            user_id=user.id, organization_id=org_id, role="team_member"
        )
        db.session.add(user_org_role)
        db.session.commit()
    ...
    # Audit log
    AuditLog.log_action(
        user.id, org_id, "login", "user", user.id, details={"provider": provider}
    )
