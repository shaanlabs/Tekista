...
class Organization(db.Model):
    ...
    users = db.relationship(
        "User", backref="organization", lazy="dynamic", cascade="all, delete-orphan"
    )
    projects = db.relationship(
        "Project", backref="organization", lazy="dynamic", cascade="all, delete-orphan"
    )
    api_tokens = db.relationship(
        "APIToken", backref="organization", lazy="dynamic", cascade="all, delete-orphan"
    )
    audit_logs = db.relationship(
        "AuditLog", backref="organization", lazy="dynamic", cascade="all, delete-orphan"
    )
...
class Webhook(db.Model):
    ...
    integration_id = db.Column(
        db.Integer, db.ForeignKey("integration.id"), nullable=False
    )
